...
internal sealed class CpuSieveBackend : ISieveBackend
{
    ...
    public ulong ExecuteSome(GapReport grl, ulong loopMinCheckedValue,
        uint[] fullDivisorList,
        uint[] offsets,
        ulong divisorsFillPosition,
        ulong divisorPosition,
        byte[] sieveBytes,
        TimeSpan preworkTime)
    {
        var done = DateTime.UtcNow + preworkTime;
        for (; divisorPosition < divisorsFillPosition; divisorPosition++)
        {
            if (DateTime.UtcNow >= done)
                break;

            ProcessPrimeOffsets(loopMinCheckedValue, fullDivisorList, offsets, divisorPosition, sieveBytes);
        }

        return divisorPosition;
    }
    ...
    private void ProcessPrimeOffsets(ulong loopMinCheckedValue, uint[] fullDivisorList, uint[] offsets,
