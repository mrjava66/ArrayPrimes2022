...
//take direction as to which block to hit.
//assembler for MarkAnArray
//each thread hits multiple blocks in a row to minimize calls to 'GetNextMark'

//return and save an arrays of gap totals.
//return and save an arrays of first gaps.

//implement n^(1/3) and binary-composites.

namespace ArrayPrimes
{
    internal class Program
    {
        ...
        {
            const ulong anvilSize = 3 * 5 * 7 * 11 * 13 * 17 * 19 * 23;
            const ulong fullAnvilSize = anvilSize + Two28;
            _anvil = new byte[fullAnvilSize];
        ...
        private static void ConfigureSystem()
        {
            ...
            var blockOrder = ConfigurationManager.AppSettings["BlockOrder"];

            if (blockOrder != "Linear") _runAllBlocksInOrder = false;

            _quickCheck = ConfigurationManager.AppSettings["QuickCheck"];

            var allowQuickCheckBailout = ConfigurationManager.AppSettings["AllowQuickCheckBailout"];
            _allowQuickCheckBailout = bool.TryParse(allowQuickCheckBailout, out var val) ? val : true;
            ...
            uint[] offsets, ulong divisorsFillPosition, ulong lastCheckedPrime)
        {
            ...
            for (var a = v1; (a < v2) || (v2 == 0 && a == v1); a++)
            {
                ...
                Buffer.BlockCopy(_anvil, offset, bytes00, 0, (int)Two28);
